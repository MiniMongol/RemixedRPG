...
return item.init{
    ...
    info = function(self,item)
      ...
      str = stra-item:level()
      local info = "Боевой топор с утяжелителем, в виде шара, в полотне возле носка.\n\nГасок - оружие ближнего боя "..tier.." порядка. Средний урон составляет "..math.ceil((maxDmg+minDmg+tier*item:level()*2)/2).." единиц за удар и, как правило, требует "..stra.." очков силы. Это довольно меткое оружие.\n\n"..self.data.sInfo
