...
return item.init{
    ...
    info = function(self,item)
      ...
      str = stra-item:level()
      local info = "Этот увесистый меч должен быть в состоянии разрубить всё что угодно. \n\nКлеймор - оружие ближнего боя "..tostring(tier).." порядка. Средний урон составляет "..tostring(math.ceil((maxDmg+minDmg+tier*item:level()*2)/2)).." единиц за удар и, как правило, требует "..stra.." очков силы.\n\n"..self.data.sInfo
