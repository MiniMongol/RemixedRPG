...
return item.init{
    ...
    info = function(self,item)
      ...
      str = stra-item:level()
      local info = "Древковое оружие с лезвием от меча на конце древка.\n\nГлефа - оружие ближнего боя "..tostring(tier).." порядка. Средний урон составляет "..math.ceil((maxDmg+minDmg+tier*item:level()*2)/2).." единиц за удар и, как правило, требует "..stra.." очков силы. Это довольно медленное, но меткое оружие.\n\n"..self.data.sInfo
