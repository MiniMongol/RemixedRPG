...
return item.init{
    ...
    info = function(self,item)
      ...
      str = stra-item:level()
      local info = "На вид чуть больше боевого посоха, но не ожидаемо тяжелый. А также металл, из которого он был изготовлен, вам не знаком.\n\nРюй Джингу(Копия) - оружие ближнего боя "..tier.." порядка. Средний урон составляет "..math.ceil((maxDmg+minDmg+tier*item:level()*2)/2).." единиц за удар и, как правило, требует "..stra.." очков силы. Это довольно быстрое и меткое оружие.\n\n"..self.data.sInfo
